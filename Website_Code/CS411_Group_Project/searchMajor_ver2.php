...
<?php
	...
	if($num_rows > 0){
	    ...
	    echo "<table border = '2' style = 'margin:0px auto'><tr><td>Year</td><td>Term</td><td>Subject</td><td>Number</td><td>A+</td><td>A</td><td>A-</td><td>B+</td><td>B</td><td>B-</td><td>C+</td><td>C</td><td>C-</td><td>D+</td><td>D</td><td>D-</td><td>F</td><td>W</td><td>Primary Instructor</td></tr>";
	    ...
	    $sql_sec = "SELECT DISTINCT `Group` FROM Major_Course WHERE Subject = '$sub' AND Number = '$num' AND Department = '$major'";

	    $glist = $mysqli->query($sql_sec);

	    $num_glist = $glist->num_rows;

	    echo "<br></br>";

	    if($num_glist > 0){
	        $sql3 = "SELECT C1.Subject, C1.Number, AVG(`Average Grade`) AS Ave_Grade, (SUM(`A+`)+SUM(`A`))/SUM(`Students`) AS A_Ratio, COALESCE(AVG(`Score`), 0) AS Course_Rate, SUM(`W`)/SUM(`Students`) AS Drop_ratio, (AVG(`Average Grade`)*0.3/5+((SUM(`A+`)+SUM(`A`))/SUM(`Students`))*0.2+COALESCE(AVG(`Score`), 2.5)*0.2/5 + (1-(SUM(`W`)/SUM(`Students`)))*0.3) AS Final_Score FROM Courses_test AS C1 NATURAL JOIN Major_Course AS C2 LEFT JOIN Rating AS R1 ON C1.Subject = R1.Subject AND C1.Number = R1.Number WHERE `Group` IN (SELECT DISTINCT `Group` FROM Major_Course WHERE Subject = '$sub' AND Number = '$num' AND Department = '$major') AND C2.Department = '$major' AND (C1.Subject != '$sub' OR C1.Number != '$num') GROUP BY Subject, Number ORDER BY Final_Score DESC LIMIT 10";
	        ...
	        print("This is not a optional major course.");
	    }
	    $result->free();

	}
	else{
	    print("Do not find the course.");
	}
